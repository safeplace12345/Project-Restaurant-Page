...
const mapOverArray = (arr) => {
    arr.map(element => {
        ...
        element.foodType = `<h6 class="card-text text-success">${!element.foodType? "Spicy" : element.foodType}</h6>`
        element.protein = `<p class="card-text">${!element.protein ? "Natural" : element.protein}</p>`
...
const newCards = (element) => {
    ...
    newcard.innerHTML += element.foodName + img + element.foodType + element.protein + element.calories
    return newcard;
}
...
export function autoMenu(element) {
    if (window.document.location.hash === "#menu")
        fetchMenu(element);
